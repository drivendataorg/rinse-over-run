...
import weighted
from src.features.utils import mape,to_log,from_log,from_sqrt,to_sqrt
import pickle
...
def create_new_cols(data):
    ...
    data['duration' ]= pd.to_numeric \
        (data.groupby('process_id').apply(lambda x :  (x['timestamp' ] -x['timestamp'].min()))).values
...
def train_model_1(datasets,labels):
    ...
    categorical_feat = ['pipeline', 'object_id', 'pre_rinse', 'caustic', 'intermediate_rinse', 'acid', 'final_rinse',
                        'phase']
    ...
    for key_fold, fold in enumerate(folds):
        ...
        with open(PERSISTED_MODEL_PATH+'model_l1_1.pickle', 'wb') as handle:
