...
class BaseTrainer:
    ...
    def __init__(self, model, loss, metrics, resume, config, train_logger=None):
        self.config = config
        self.logger = Logger(self.__class__.__name__).logger  # logging.getLogger(self.__class__.__name__)
        self.model = model
        self.loss = loss
        ...
        self.logger.info('%d non_frozen_parameters ' % len(non_frozen_parameters))
        ...
        for state in self.optimizer.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to(device=torch.device(self.device))
        self.train_logger = checkpoint['logger']
        self.config = checkpoint['config']
        self.logger.info("Checkpoint '{}' (epoch {}) loaded".format(resume_path, self.start_epoch))


def print_training_log(batch_idx, epoch, loss, verbosity, logger, log_step, data_loader):
    if verbosity >= 2 and batch_idx % log_step == 0:
